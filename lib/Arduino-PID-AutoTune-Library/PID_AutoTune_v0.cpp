...
int PID_ATune::Runtime()
{
	...
	if(refVal>setpoint+noiseBand) *output = *output-oStep;

	else if (refVal<setpoint-noiseBand) *output = *output+oStep;
  ...
  if(isMax)
  {
    ...
    peak1 = now;
	 if(peakCount<10) {
		...
		Serial.printf("Added peak %i at %f (max)\r\n", peakCount, refVal);
	 }	
  }
  else if(isMin)
  {
    ...
    if(peakCount<10) {
		peaks[peakCount] = refVal;
		Serial.printf("Added peak %i at %f (min)\r\n", peakCount, refVal);
	}
  }
